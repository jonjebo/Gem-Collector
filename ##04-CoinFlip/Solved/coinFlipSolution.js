...
//create flip coin function for button. then define an input (result)
function flipThatCoin(result) {

  // STEP ONE:
  // Declare and initialize a variable named randomNumber to either 0 or 1. Make it random.
  var randomNumber = Math.floor(Math.random() * 2); //use two because you want it between 0 and 1. Want to use one number higher because with 2 it will only max at 1.9999999
  ...
  if (result === randomNumber) {
    wins++;
    $("#win-lose").html("<h2>Winner!</h2>");//updates wins and displays winner! in the html <h2>
    $("#wins").html("<h3>" + wins + "</h3>");//updates area where user is gaining score for winning
  }
